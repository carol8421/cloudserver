...
function putData(request, response, bucketInfo, objMd, log, callback) {
    ...
    // TODO: Check additional methods for the same case.
    const context = {
        bucketName: request.bucketName,
        owner: canonicalID,
        namespace: NAMESPACE,
    ...
    return async.waterfall([next => auth.server.doAuth(
        ...
        (userInfo, next) => {
            if (useMultipleBackend &&
                request.headers['x-scal-is-nfs'] !== 'true') {
        ...
        (bucketInfo, objMd, next) => {
            const reqContentMD5 = request.headers['content-md5'];
            if ((bucketInfo && bucketInfo.isNFS()) &&
                (objMd && reqContentMD5) &&
                (objMd['content-md5'] !== reqContentMD5)) {
                return next(errors.BadDigest);
            }
            if (useMultipleBackend) {
